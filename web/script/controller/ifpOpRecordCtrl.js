...
define([
    ...
    app.controller("ifpOpRecordCtrl", ["$rootScope", "$scope", "$http", "$location", "$filter", "$q", "$stateParams", "$uibModal", "IFPAPIService", "IFPJumpService",
    function ($rootScope, $scope, $http, $location, $filter, $q, $stateParams, $uibModal, IFPAPIService, IFPJumpService, undefined) {

        var tType = $location.search()["tasktype"],
            isDouble = $scope.isDouble = $location.search()["ctrl"] == "ifpOpRecordDouble",
            isBg = $location.$$path == "/bgOpRecord";//小二后台
        ...
        */
        function search(condition) {
            ...
            $http({
                method: "post",
                url: IFPAPIService.ifpBaseUrl + "policy/getTaskProgressList",
                timeout: 10000,
                data: validCondition
        ...
        */
        function searchResult(data) {
            if (data.success) {
                ...
                alert(_.map(data.errMsg, function (item, key) { return item }).join("<br>") || "查询失败").then(function () {
        ...
        $scope.downloadClick = function (url, id) {
            window.open(IFPAPIService.ifpBaseUrl + "policy/downloadFile?id=" + id + "&url=" + url);
        }
        ...
        function checkPermition() {
            ...
            $http({
                method: "post",
                url: IFPAPIService.ifpBaseUrl + "policy/checkPermition",
                timeout: 10000,
